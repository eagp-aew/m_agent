...
- Status: YELLOW
- Active work package: WP-0031-reflection-weekly-review (ACCEPTED after the human-authorized fourth heading-parser repair and fresh independent verifier PASS); latest accepted: WP-0031-reflection-weekly-review
...
- `personal-co/App.tsx` provides responsive Expo Web surfaces for Chat, Core Memory, Learning, Reflection, Memory Changes, Archive, Import, and Settings.
- `personal-co/src/services/letta.ts` owns typed Letta client access, health-only connection probing, exact registered generation/embedding handle preflight before Agent operations, exact tagged-agent reuse, memory-schema validation, messaging, archival search and deletion, exact-term forget, temporary-session reconciliation, and transactional same-ID generation-model switching with callback-scoped workflow leases, fail-closed Archive pagination, invariant verification, compensation, and retained write lock after unverifiable rollback.
- `personal-co/src/domain/` contains testable memory, epistemic, import, guided-learning episode, cross-domain reflection, falsifiable hypothesis, weekly-review, single-agent, change-governance, privacy, portable-snapshot, and model-switch invariant rules.
...
- `personal-co/tests/` covers fixed memory blocks, protected writes, archive-first imports, six-stage learning episodes, transparent reflection connections and hypotheses, exact-five-section coaching, Archive-first weekly reviews, all-core pending proposals, evidence-state gates, immutable provenance, exact Archive read-back, duplicate-agent rejection, exact-handle matching, real adapter ordering and fail-closed inventory errors, existing-schema validation, same-ID updates, agent-bound proposals, exact forget, temporary privacy, import provenance, secret-free same-agent snapshot restore, workflow-scoped model-switch barriers, complete pagination, invariants, compensation, and rollback locking.
...
- Risk: A typed adapter can still differ from a live Letta deployment, its registered handles, or its pagination/update semantics.
  - Mitigation: WP-0028 fails closed on exact registered handles and WP-0029 verifies the application-layer transaction; run T-01/T-03/T-04/T-05/T-10 against the user's self-hosted Letta server before calling the full V1 complete.
- Risk: The model-switch write barrier covers one adapter instance, not other tabs, processes, or direct Letta clients.
  - Mitigation: Fresh post-update invariants and fail-closed rollback detect local snapshot drift, but a server-side maintenance lock or operator write pause is still required for live multi-client switching.
...
- Risk: Live Letta may normalize Archive tags or timestamps differently from the deterministic learning persistence contract.
  - Mitigation: WP-0030 and WP-0031 report success only after new exact Archive records, expected Block values or pending proposals, and Agent binding are read back; run live T-06 before claiming deployment acceptance.
...
| 2026-09-07 | WP-0029-transactional-model-switch | Added explicit same-Agent generation-model switching with callback-scoped write leases, complete snapshots, embedding and memory invariants, verified compensation, retained failure lock, and responsive Settings controls | Independent verifier PASS for 9/9 criteria, targeted 13/13 and full 40/40 tests, typecheck, Web export, 390px/desktop browser QA, 29/29 protocol checks, report gate, and pre-accept gate |
| 2026-09-07 | WP-0030-guided-learning-episodes | Added the six-stage Learning surface, no-write coaching, evidence-gated learning episodes, Archive-first persistence, exact new-record read-back, and one current LEARNING_MODEL entry per concept | Independent verifier PASS for 10/10 criteria, targeted 15/15 and full 55/55 tests, typecheck, Web export, desktop/390px browser QA, 29/29 protocol checks, report gate, and pre-accept gate |
| 2026-09-09 | WP-0031-reflection-weekly-review | Added transparent cross-domain connections, falsifiable growth hypotheses, exact five-section user-triggered weekly reviews, Archive-first same-Agent persistence, and confirmation-only all-core proposals | Independent verifier PASS for 11/11 criteria, targeted 30/30 and full 73/73 tests, typecheck, Web export, desktop/390px evidence, 29/29 protocol checks, report gate, and pre-accept gate |

## Open questions

- OD-0028 remains open: pushing the accepted WP-0031 milestone requires explicit confirmation of the final commit SHA, `https://github.com/eagp-aew/m_agent.git`, branch `codex/personal-co-v1`, and inclusion of Personal Co source plus internal `.ai` project records.

## Next recommended work

1. Create the authorized local WP-0031 milestone commit, then obtain exact-SHA authorization before pushing Personal Co source and internal `.ai` records to `https://github.com/eagp-aew/m_agent.git` on `codex/personal-co-v1`.
2. Connect the accepted application flows to the user's self-hosted Letta deployment with registered handles and run live T-01/T-03/T-04/T-05/T-06/T-10 create/reuse/message/archive/learning/reflection/model-switch checks.
3. Plan and prove PostgreSQL/pgvector snapshot and recovery separately from the portable application snapshot.
