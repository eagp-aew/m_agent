...
| Personal Co browser smoke | `cd personal-co && npm run web` | Responsive UI changes | Manually exercise the six surfaces at desktop and 390px narrow width, including the full model-switch card and disabled persistent-memory controls while offline, connecting, and switching. |
| Live Letta integration | Not yet automated | Letta server, agent lifecycle, messaging, archive, forget, restore, privacy reconciliation, or model-handle changes | Requires a reachable self-hosted Letta server, registered model/embedding handles, live T-01/T-03/T-04/T-05/T-10 execution, and a separate PostgreSQL snapshot/recovery proof; deterministic adapter tests do not establish this boundary. |
