...
|---|---|---|---|
| Personal Co model-switch tests | `cd personal-co && node --test tests/model-switch.test.mjs` | Model-switch, write-barrier, pagination, invariant, rollback, or Settings-state changes | Requires 13 focused cases covering reconnect fail-closed behavior, same-ID success, embedding/no-op rejection, direct and workflow-scoped barriers, leaked-capability invalidation, ambiguous pagination, Block-limit drift, verified compensation, and retained rollback lock. |
| Personal Co guided-learning tests | `cd personal-co && node --test tests/learning-workflow.test.mjs` | Learning stages, evidence gates, coaching, Archive/LEARNING_MODEL persistence, or Learning UI orchestration changes | Requires 15 focused cases covering state ceilings, immutable factory episodes, clone rejection, question bounds, serialization, concept upsert/limits, no-write reconciliation, exact-Agent ordering, exact new Archive read-back, and honest partial failures. |
| Personal Co domain tests | `cd personal-co && npm test` | Any application behavior change | Covers fixed memory, confirmation gates, archive imports, guided-learning evidence and persistence, exact registered model/embedding handles, real adapter preflight ordering and sanitized failures, tagged-agent selection, exact existing schema, same-ID transactional updates, workflow barriers, complete pagination, rollback, agent-bound proposals, exact forget, privacy reconciliation, provenance, and portable snapshots. |
...
| Personal Co browser smoke | `cd personal-co && npm run web` | Responsive UI changes | Manually exercise all seven surfaces at desktop and 390px narrow width, including the six-stage Learning form, separate coaching output, state preview, full model-switch card, and disabled persistent-memory controls while offline, connecting, learning, and switching. |
| Live Letta integration | Not yet automated | Letta server, agent lifecycle, messaging, archive, learning, forget, restore, privacy reconciliation, or model-handle changes | Requires a reachable self-hosted Letta server, registered model/embedding handles, live T-01/T-03/T-04/T-05/T-06/T-10 execution, and a separate PostgreSQL snapshot/recovery proof; deterministic adapter tests do not establish this boundary. |
| V1 completion audit evidence | `python3 -B scripts/protocol_gate.py pre-accept WP-0027-v1-completion-audit --report .ai/AGENT_REPORTS/WP-0027-v1-completion-audit.md` | When V1 completeness claims or the next package boundary change | Confirms the verified 41-row/T-01–T-12 audit remains replayable and does not promote local evidence into live-server or database-backup proof. |
