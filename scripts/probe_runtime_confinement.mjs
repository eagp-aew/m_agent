...
import { startOwned, validateEndpoint, listenerGone } from '../personal-co/server/runtime-process.mjs';
export { startOwned, validateEndpoint, listenerGone } from '../personal-co/server/runtime-process.mjs';
...
export async function sourceDigest() {
  const sources = await Promise.all([fs.readFile(new URL('../personal-co/server/runtime-sandbox.mjs', import.meta.url)),
    ...
    fs.readFile(new URL('../personal-co/server/runtime-process.mjs', import.meta.url))]);
